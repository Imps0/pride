...
class DetectorMalware:
    def __init__(self, scr_path):
        #escaneamento YARA local
        matches = yara_scanner.scan_file(src_path)
        if matches:
            print(f"[YARA] ALERTA! {src_path} corresponde às regras: {matches}")
        
        # consulta via hash no MalwareBazaar
        try:
            sha256 = Hash(src_path).gerar_Hash()
            malwareCheck = ColetaDados()
            malwareCheck.dataBase_Search(sha256)

            if malwareCheck.malware:
                malwareCheck.DisplayData()
        except Exception as e:
            print(f"[DetectorMalware] Erro ao processar {src_path}: {e}")
